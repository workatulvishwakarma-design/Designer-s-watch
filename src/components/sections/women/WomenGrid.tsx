...
import { getCollectionProductsByGender } from "@/actions/public.actions";
import type { UnifiedProduct } from "@/lib/products";
import { sortProducts, type SortOption } from "@/lib/products";
import { Loader2 } from "lucide-react";

const categories = ["All", "Classic", "Minimal", "Gold", "Silver", "Rose Gold"];
...
export default function WomenGrid() {
    ...
    useEffect(() => {
        async function load() {
            try {
                const data = await getCollectionProductsByGender("Women");
                setProducts(data);
            } catch (err) {
                console.error("Failed to load Women's products:", err);
            } finally {
                setIsLoading(false);
            }
        }
        ...
        >
            ...
            <div className="text-center pb-6 px-6">
                ...
                <p className="font-cormorant italic text-lg md:text-xl text-[#003926]">
                    &quot;Sculpted Elegance & Uncompromised Grace&quot;
                </p>
            </div>
            <FilterBar
